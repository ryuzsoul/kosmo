...
public class FileUtil {
	...
	//파일 다운로드 로직 원본파일명과 저장된 파일명을 지정하여 파일 다운로드시
	//원본파일명으로 변경한다
	/**
	 * 
	 * @param request
	 * @param response
	 * @param directory
	 * @param sfileName 서버에 저장된 파일명
	 * @param ofileName 첨푸파일의 원본 파일명
	 */
	public static void download2(
		...
		*/
		try{
			...
			/*
			4-3. 응답헤더명 : Content-Disposition
				응답헤더명에 따른 값 : attachment;sfileName=파일명
				setHeader(응답헤더명,헤더값)으로 추가함
			브라우저 종류에 따라 한글파일명이 깨지는 경우가 있음으로
			브라우저별 인코딩 방식을 달리하는것임(파일명을 인코딩)
			*/
